...
import eu.starify.plots.gui.PlotGUI;
...
import eu.starify.plots.utils.Utils;
...
public class PlotCommand implements CommandExecutor {
    ...
    public PlotCommand(SPlots plugin)
    {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        final LanguageManager languageManager = SPlots.getInstance().getLanguageManager();
        ...
        else if(args.length == 1)
        {
            if(args[0].equalsIgnoreCase("info"))
            {
                ...
                if(plot != null)
                {
                    player.sendMessage(ChatUtil.fixColors("&8&m----------&8[ &d&lS&9&lPlot &8]&8&m----------"));
            ...
            } else if(args[0].equalsIgnoreCase("autorzy") || args[0].equalsIgnoreCase("authors")) {
                player.sendMessage(ChatUtil.fixColorsWithPrefix("&7Autorzy forka:"));
                player.sendMessage(ChatUtil.fixColorsWithPrefix("AspDev, Rigolleto"));
                player.sendMessage("");
                player.sendMessage(ChatUtil.fixColorsWithPrefix("&aGłówny author:"));
                player.sendMessage(ChatUtil.fixColorsWithPrefix("&7https://github.com/Symii"));
            }
            ...
            {
                if(SPlots.getInstance().getPlotManager().hasPlot(player)) {
                ...
                if(invite_players.containsKey(player_name))
                {
                    ...
                    Bukkit.getScheduler().scheduleSyncDelayedTask(SPlots.getInstance(), () -> {
        ...
        else if(args.length == 2)
        {
            ...
            else if(args[0].equalsIgnoreCase("zapros") || args[0].equalsIgnoreCase("invite"))
            {
                ...
                if(target != null && target.isOnline())
                {
                    if(SPlots.getInstance().getPlotManager().hasPlot(target)) {
            ...
            else if(args[0].equalsIgnoreCase("stworz") || args[0].equalsIgnoreCase("create"))
            {
                ...
                long player_money = SPlots.getInstance().getPlayerMoney(player);
                ...
                SPlots.getInstance().takePlayerMoney(player, plot_create_cost);

                PlotManager plotManager = SPlots.getInstance().getPlotManager();
    ...
    {
        LanguageManager languageManager = SPlots.getInstance().getLanguageManager();

        player.sendMessage(ChatUtil.fixColors("&8&m-&3|&8&m---------&3[ &d&lS&9&lPlots &3]&8&m---------&3|&8&m-"));
        ...
        player.sendMessage(languageManager.getMessage("help-plot-authors"));
